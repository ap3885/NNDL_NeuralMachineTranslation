...
def encoding_layer(rnn_inputs, rnn_size, num_layers, keep_prob,
    ...
    ######################################################################################    
    #create recurrent layer (lstm + dropout) according to lstem/gru specification by the user
    def create_cell_fw(rnn_size):
        with tf.variable_scope("create_cell_fw"):
            if cell_type == 'LSTM':
                cell = tf.contrib.rnn.LSTMCell(rnn_size, initializer=tf.random_uniform_initializer(-0.1,0.1,seed=2), reuse=False) 
            elif cell_type == 'GRU':
                cell = tf.contrib.rnn.GRUCell(rnn_size)
            drop = tf.contrib.rnn.DropoutWrapper(cell, output_keep_prob = keep_prob)
        return drop        
    
    #create RNN cell of the corresponding number of layers
    enc_cell_fw = tf.contrib.rnn.MultiRNNCell([create_cell_fw(rnn_size) for _ in range (num_layers)])
       
    return tf.nn.dynamic_rnn(enc_cell_fw, enc_embed, dtype=tf.float32)
...
def decoding_layer(dec_input, encoder_state,
    ...
    ######################################################################################
    #tensor to map decoder input to the decoder embeddings
    dec_embeddings = tf.Variable(tf.random_normal([target_vocab_size, decoding_embedding_size]))
    #decoder input mapped to the embedding space
    dec_embed_input = tf.nn.embedding_lookup(dec_embeddings, dec_input)
    
    #create cell according to lstm/gru specified by the user
    def cell():
        if cell_type == 'LSTM':
    ...
    #create decoding cell layer corresponding to num_layer
...
def my_optimizer(loss,grad_clip, learning_rate):
    ...
    #######################################################
    #using adam optimizer because its the best
    optimizer = tf.train.AdamOptimizer(learning_rate)
    train = optimizer.minimize(loss)
    
    #applying gradient clipping
    gradients = optimizer.compute_gradients(loss)
    clipped_gradients = [(tf.clip_by_value(grad, -grad_clip, grad_clip), var) for grad, var in  gradients if grad is not None]
